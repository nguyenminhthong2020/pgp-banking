...
router.post("/partner/recharge", async function (req, res) {
  ...
  const cleartext_t = signature.replace(/\\n/g,"\n");
  const cleartext = cleartext_t.replace(/\\"/g,"\"");
  // Verify
  const verified = await openpgp.verify({
    message: await openpgp.cleartext.readArmored(cleartext),           // parse armored message
    publicKeys: (await openpgp.key.readArmored(partner.PGP_PUBLICKEY)).keys // for verification
  });
  const veri = verified.signatures[0].valid;
  // console.log(verified);
  console.log("my veri: ", veri);
  // Kiểm tra nội dung chữ kí có khác req.body hay không
  const contentSignature = (await openpgp.cleartext.readArmored(cleartext)).getText();
  if (contentSignature != JSON.stringify(req.body)) {
    veri = false;
  }
  ...
  try {
    ...
    const entityUpdateLog = {
      bank_code: req.get("partnerCode"),
      receive_account_number : req.body.account_numer,
      money : moneySend,
      created_at: moment().format('YYYY-MM-DD HH:mm:ss'),
    }
